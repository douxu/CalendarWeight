...
public class ScheduleView extends Activity {
	...
	private BorderEditText scheduleText = null;
	...
	public ScheduleView() {
		//lc = new LunarCalendar();   //LunarCalendar表示阴历
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		scheduleText = (BorderEditText) findViewById(R.id.scheduleText);
		scheduleText.setBackgroundColor(Color.WHITE);
		if(schText != null){
			//在选择日程类型之前已经输入了日程的信息，则在跳转到选择日程类型之前应当将日程信息保存到schText中，
			//当返回时再次可以取得。
		...
		scheduleSave.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				if(TextUtils.isEmpty(scheduleText.getText().toString())){
					...
					//将scheduleID保存到数据中(因为在MainActivity中点击一个日期可能会对应多个标记日程(scheduleID))
	...
	public void setScheduleDateTag(int remindID, String year, String month, String day,int scheduleID){
		...
		if(remindID >= 0 && remindID <= 3){
			...
			//将时间推迟，第一个参数是推迟的类型，第二份参数是推迟的时间，calendar.DATE是按天推迟
				}else{
	...
	public String getScheduleDate() {
		...
		week = scheduleDate.get(3);  //小于10前面加0
